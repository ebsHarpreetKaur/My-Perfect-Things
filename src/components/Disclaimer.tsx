...
const Disclaimer = () => {
  ...
  // useEffect(() => {
  //   pages().then(response => {
  //     setDisclaimer(response[0])
  //     // console.log("response dis", response[0])
  //   }
  //   )
  // }, [])

  const { pagelist } = useSelector((state) => state.pagesReducer);
  const disclaimerarray = pagelist[0]
  // console.log('pagelist', disclaimerarray);
  ...
  const source = {
    html: `${disclaimerarray === undefined ? "Loading..." : disclaimerarray?.content?.rendered}`
  };
  ...
  return (
    <SafeAreaView style={styles.container4}>
      ...
      <ScrollView style={styles.scrollView}>
        {/* <Text style={styles.itemText6}>{disclaimerarray?.title?.rendered}</Text> */}
